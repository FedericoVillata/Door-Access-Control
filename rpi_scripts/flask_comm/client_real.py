...
id_info = { "company_name" : "Amazon", "room" : "expo" }
...
def fetch_data(number_to_send):
    url = 'http://127.0.0.1:5000/api/rpi_token'  # URL del server Flask
    data_ = read_json_file(id_path)
    #number_to_send = "f899139df5e1059396431415e770c6dd"
    #number_to_send = "csjdkjkddjn"
...
def show_popup(data):
    ...
    bg_color = 'green' if data['status'] else 'red'
    ...
    title_label = tk.Label(popup, text=f"Title: {data['username']}", bg=bg_color)
    title_label.pack(pady=10)

    body_label = tk.Label(popup, text=f"Body: {data['username']}", bg=bg_color)
    body_label.pack(pady=10)

    # Visualizzare la stringa generata
    random_string_label = tk.Label(popup, text=f"Random String: {data['room']}", bg=bg_color)
